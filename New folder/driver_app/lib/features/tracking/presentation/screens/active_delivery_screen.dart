...
class _ActiveDeliveryScreenState extends ConsumerState<ActiveDeliveryScreen>
  ...
  Future<void> _openNavigation({
    ...
    final livePosition = await ref
        .read(locationServiceProvider)
        .getCurrentPosition();
    final resolvedOriginLatitude = livePosition?.latitude ?? originLatitude;
    final resolvedOriginLongitude = livePosition?.longitude ?? originLongitude;
    ...
    return Scaffold(
      ...
      body: orderState.when(
        ...
        data: (order) {
          ...
          return ListView(
            padding: EdgeInsets.only(bottom: 168 + bottomInset),
            children: [
              _DeliveryMapSection(
                customerLatLng: customerLatLng,
                driverLatLng: driverLatLng,
...
class _DeliveryActionBar extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final hasStageAction = switch (currentStage) {
      'accepted' || 'on_the_way' || 'arrived' => true,
      _ => false,
    };
    final canNavigate = switch (currentStage) {
      'on_the_way' || 'arrived' || 'delivered' => true,
      _ => false,
    };
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
                ...
                children: [
                  Expanded(
                    child: OutlinedButton.icon(
                      ...
                      label: Text(context.strings.callCustomer),
                  ...
                  Expanded(
                    child: FilledButton.icon(
                      ...
                      label: Text(context.strings.navigate),
                      style: FilledButton.styleFrom(
                        minimumSize: const Size.fromHeight(50),
                        textStyle: const TextStyle(
                          fontSize: 14,
              ...
              if (!canNavigate) ...[
                ...
                Text(
                  context.strings.isArabic
                      ? 'يتفعّل زر الملاحة بعد الضغط على "ابدأ الرحلة".'
                      : 'Navigation becomes available after tapping "Start trip".',
                  textAlign: TextAlign.center,
                  style: Theme.of(context).textTheme.bodySmall?.copyWith(
              ...
              if (hasStageAction) ...[
                const SizedBox(height: 10),
                _StageActions(currentStage: currentStage, onAdvance: onAdvance),
              ],
            ],
          ),
        ),
