...
class DeviceLocationService {
  static const double _targetAccuracyMeters = 5;
  static const double _goodAccuracyMeters = 20;
  static const int _maxCurrentPositionAttempts = 5;
  ...
  Future<_ResolvedPosition?> _resolveBestPosition({
    ...
    for (var attempt = 0; attempt < _maxCurrentPositionAttempts; attempt++) {
      final fix = await _tryCurrentPosition(
        timeout: Duration(seconds: attempt == 0 ? 10 : 7),
      );

      if (fix != null) {
        final resolvedFix = _ResolvedPosition(
          position: fix,
          isFromLastKnown: false,
        );
        candidates.add(resolvedFix);

        final accuracy = _safeAccuracy(fix.accuracy);
        final age = _positionAge(fix);
        if (accuracy != null &&
            accuracy <= _targetAccuracyMeters &&
            age <= const Duration(seconds: 30)) {
          return resolvedFix;
        }
      }

      final best = _pickBestPosition(candidates);
      if (!_needsAccuracyRefinement(best)) {
        return best;
      }
