...
class CustomerAppController extends Notifier<CustomerAppState> {
  ...
  Future<OrderModel?> createOrder({
    ...
    final nextOrders = state.orders.any((item) => item.orderId == order.orderId)
        ? state.orders
    ...
    var order = OrderModel(
      ...
      items: validItems
          .map(
            (item) => OrderLineItem(
              id:
                  'local-${item.product.id}-${DateTime.now().millisecondsSinceEpoch}',
              orderId: remoteReceipt.orderId,
              productId: item.product.id,
  ...
  void _applyLiveOrderPatch(_LiveOrderPatch patch) {
    ...
    if (orderIndex == -1) {
      final parentIndex = state.orders.indexWhere(
        (order) => order.childOrders.any((child) => child.orderId == patch.orderId),
      );

      if (parentIndex != -1) {
        final parentOrder = state.orders[parentIndex];
        final nextChildren = parentOrder.childOrders.map((childOrder) {
          ...
          return childOrder.copyWith(
            ...
            address: nextAddress,
            driver: nextDriver,
            driverLatitude: patch.driverLatitude ?? childOrder.driverLatitude,
            driverLongitude: patch.driverLongitude ?? childOrder.driverLongitude,
            ...
            routeDurationSeconds:
                patch.routeDurationSeconds ?? childOrder.routeDurationSeconds,
    ...
    final updatedOrder = currentOrder.copyWith(
      ...
      fulfillmentType: patchFulfillmentType,
      fulfillmentInventoryCode:
          patchFulfillmentInventoryCode ?? currentOrder.fulfillmentInventoryCode,
    );
  ...
  OrderModel? _buildOrderFromPatch(_LiveOrderPatch patch) {
    ...
    final subtotal = _calculateSubtotalFromItems(items) ?? (product.priceOmr * quantity);
    final remoteTotal =
    ...
    return rawItems
        ...
        .map((item) {
          ...
          return OrderLineItem(
            ...
            deliveryFee:
                _parseNumber(item['deliveryFee'] ?? item['delivery_fee']),
